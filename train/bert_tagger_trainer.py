...
class BertSequenceLabeling(pl.LightningModule):
    def __init__(
        ...
        self.task_idx2label = {label_idx: label_item for label_idx, label_item
                               in enumerate(get_labels(self.args.data_sign))}
        ...
        self.tokenizer = AutoTokenizer.from_pretrained(args.bert_config_dir,
                                                       use_fast=False,
    ...
    def add_model_specific_args(parent_parser: argparse.ArgumentParser) -> argparse.ArgumentParser:
        ...
        parser.add_argument("--polydecay_ratio",
                            type=float,
                            default=4,
    ...
    def configure_optimizers(self) -> Tuple[List[Any], List[Dict[str, Any]]]:
        ...
        t_total = (len(self.train_dataloader()) // (self.args.accumulate_grad_batches * num_gpus)
    ...
    def test_epoch_end(self, outputs: List[Dict[str, torch.Tensor]]) -> Dict[str, Dict[str, Any]]:
        ...
        self.result_logger.info(f"EVAL INFO -> test_f1 is: {span_f1}, test_precision is: {span_precision}, "
                                f"test_recall is: {span_recall}")
